...
## #2
### Задание
Напиши функцию на php, которая на входе принимает номер банковской карты, а на выходе возвращает булевый результат проверки корректности ее номера, используя алгоритм «Луна».
### Решение
...
```
### Решение
Ваш код может пострадать от SQL-инъекций.
...
?>
```
2. Откроем терминал и убедимся, что файл `hello_script.php` имеет права на выполнение
```
chmod +x hello_script.php
```
3. Чтобы запустить, введем в терминал
```
./hello_script.php
```
